...
public class TestUtil {
    ...
    public static boolean compareTuples(Tuple t1, Tuple t2) {
        if (t1.getTupleDesc().numFields() != t2.getTupleDesc().numFields())
        {
        		...
        		System.out.println("ffff1111111f");
            return false;
        }

        for (int i = 0; i < t1.getTupleDesc().numFields(); ++i) {System.out.println("fffff");
            if (!(t1.getTupleDesc().getFieldType(i).equals(t2.getTupleDesc().getFieldType(i))))
            {System.out.println("ff1111fff");
            System.out.println(t1.getTupleDesc().getFieldType(i));
            System.out.println(t2.getTupleDesc().getFieldType(i));
            	  return false;
            }
              
            if (!(t1.getField(i).equals(t2.getField(i))))
            {System.out.println("ff2222fff");
            	 return false;
            }
    ...
    public static void matchAllTuples(OpIterator expected, OpIterator actual) throws
        ...
        while (expected.hasNext()) {
            ...
            while (actual.hasNext()) {System.out.println("ssssss");
                Tuple next = actual.next();
                System.out.println("expectedTup"+next);
                if (compareTuples(expectedTup, next)) {System.out.println("sssvvvvvsss");
