...
public class HeapPage implements Page {
	...
	public HeapPage(HeapPageId id, byte[] data) throws IOException {
		...
		try {
			...
			for (int i = 0; i < tuples.length; i++) {
				...
				// tuples[i].setRecordId(new RecordId(pid, numSlots));
			}
		 ...
		 if (getNumEmptySlots() == 0) {
			...
			for (int i = 0; i < getNumTuples(); i++) {
				if (!isSlotUsed(i)) {
					...
					t.setRecordId(newId);
					//System.out.println("recordid"+newId.toString());
					markSlotUsed(i, true);
					tuples[i] = t;
					//System.out.println("000000000000000" + t.getRecordId());
	...
	public void markDirty(boolean dirty, TransactionId tid) {
		// some code goes here
		// not necessary for lab1
		if(dirty==true)this.dirty=tid;
		else this.dirty=null;
	}
	...
	public Iterator<Tuple> iterator() {
		Iterator<Tuple> t = new Iterator<Tuple>() {
			private int empty = getNumEmptySlots();
			private int total = getNumTuples();
			private int pos = 0;
			private int index = 0;

			@Override
			public boolean hasNext() {
				if (pos < total - empty && index < getNumTuples()) {
					return true;
				} else
